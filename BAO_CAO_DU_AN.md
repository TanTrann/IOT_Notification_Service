...
IOT Notification Service là một microservice trong hệ thống SmartFarm. Nhiệm vụ chính là nhận thông báo do server của Phong gửi sang qua API (`POST /internal/notify`) và **đẩy push notification qua Firebase Cloud Messaging (FCM)** tới màn hình/thiết bị người dùng — hiển thị đúng nguyên nội dung, kể cả khi trang/ứng dụng đang đóng.
...
- Cần một cầu nối trung thực từ nơi sinh thông báo (server của Phong) tới màn hình hiển thị.
- Thông báo phải hiển thị **realtime** và **đánh thức được** kể cả khi màn hình/ứng dụng không ở tiền cảnh → dùng Firebase push.
- Nội dung thông báo do Phong quyết định — service không tự diễn giải lại, tránh sai lệch.
...
- **Tân** — **Push Notification Service (Firebase)**: nhận thông báo từ Phong (qua API) và đẩy push tới người dùng.
- **Thịnh** — Website quản lý SmartFarm.

Luồng dữ liệu (mô hình hiện tại):

```
Server Phong ─POST /internal/notify (JSON)─► Notification Service ─FCM─► Màn hình web kiosk cạnh cây
```

Điểm thiết kế quan trọng: server của Phong xử lý thông báo xong rồi **gọi API `POST /internal/notify`** (kèm `deviceId` trong body). Notification Service **không dịch, không biến đổi** — đẩy nguyên nội dung qua Firebase tới **đúng các màn hình đã đăng ký deviceId đó** (targeting theo thiết bị): mỗi màn hình kiosk cấu hình deviceId của cây nó đứng cạnh, chỉ nhận tin của cây đó.

> **Ghi chú tiến hoá:** phiên bản đầu của service tự "nghe ké" dữ liệu cảm biến (`xmini/sensor_data`, `xmini/control`) qua MQTT, so ngưỡng và dịch lệnh thành thông báo. Giai đoạn sau, service vẫn nghe MQTT nhưng ở topic thông báo riêng (`planttree/{deviceId}/notifications`) do Phong tự phát. Bản mới nhất **bỏ hẳn MQTT**: Phong chủ động **gọi API `POST /internal/notify`** sang service — ranh giới rõ ràng, Phong kiểm soát khi nào/gửi gì, service chỉ hiển thị. Đồng thời bỏ lớp user/đăng nhập (JWT): **deviceId trở thành danh tính của chính màn hình kiosk**, cấu hình trực tiếp trên máy — push và lịch sử đều lọc theo deviceId nên luôn khớp nhau. Các luồng dịch/so ngưỡng, kênh SSE trung gian và lớp user đã được gỡ bỏ.
...
- Cơ sở dữ liệu: MongoDB qua Mongoose (lưu FCM token + lịch sử thông báo)
- Giao thức nhận thông báo: HTTP REST (`POST /internal/notify`) — Phong gọi sang, xác thực bằng `x-api-key`
...
**Nhận thông báo qua API:** Server của Phong gọi `POST /internal/notify` (header `x-api-key`, body JSON kèm `deviceId`). Service xử lý ở `notificationService.ingest()`.

**Chuyển tiếp trung thực (không dịch):** Service giữ nguyên payload, chỉ bóc mềm dẻo các field `title`/`body`/`severity`/`type` để đặt tiêu đề/nội dung push cho đẹp; ngoài `deviceId` (bắt buộc) không field nào khác bắt buộc.

**Push qua Firebase (targeting theo device):** Mỗi tin chỉ gửi tới các token đã đăng ký **đúng deviceId** lấy từ body request (`fcmService.sendToDeviceId`) — mỗi màn hình chỉ nhận tin của cây mình. Push nổ cả khi trang/ứng dụng đóng. Token không còn hợp lệ được tự động xóa.
...
Bước 1 — Nhận: server của Phong gọi `POST /internal/notify` (body JSON kèm `deviceId`). Service xác thực `x-api-key`, lấy `deviceId` từ body, chuẩn hóa field và ghi lịch sử vào MongoDB (best-effort, chống trùng qua `eventId`).
...
Collection `notifications` (ghi khi nhận `/internal/notify`, phục vụ REST đọc của web):
- eventId, deviceId, title, body, type, severity, isRead, createdAt
- Lưu ý: `notificationService.ingest` **ghi mỗi tin vào collection này** (best-effort, không chặn push) để REST đọc lại lịch sử bền vững. Màn hình kiosk vẫn hiện realtime từ message FCM; `severity`/`type` được chuẩn hóa cho khớp enum, trùng `eventId` chỉ ghi 1 lần.
...
Nhận thông báo từ Phong (`/internal/*`):
- `POST /internal/notify` — Phong gửi thông báo vào, body `{ deviceId, title, body, type?, severity?, id?, ... }` (`deviceId` bắt buộc). Trả `202`.
...
**Chuyển tiếp trung thực:** hiển thị đúng nguyên JSON Phong gửi; client luôn có thể xem dữ liệu gốc.

**Targeting theo thiết bị:** mỗi màn hình chỉ nhận tin của cây mình (`sendToDeviceId`) — deviceId trong body request khớp deviceId token đã đăng ký. Đơn giản, không cần user/đăng nhập.
...
**FCM best-effort:** lỗi ghi DB không làm gãy việc push; token chết tự bị xóa khỏi DB.

**Chống trùng khi retry:** nếu Phong gửi kèm `id`/`eventId`, service dùng làm khóa upsert — Phong gọi lại cùng tin không tạo bản ghi lặp.

**Khởi động an toàn:** service vẫn chạy dù thiếu Firebase hoặc MongoDB — ghi log cảnh báo và tiếp tục.


## Kết Quả Đạt Được

- Service nhận thông báo qua `POST /internal/notify` và đẩy push qua Firebase tới đúng màn hình của deviceId đó.
...
- Có script `scripts/publish-test.js` giả lập Phong gọi `POST /internal/notify` để kiểm thử end-to-end.


## Hướng Phát Triển Tiếp Theo

- Phối hợp với Phong để chốt contract body của `POST /internal/notify` và cấp API key (điểm ráp nối production).
...
IOT Notification Service đảm nhiệm phần **Push Notification với Firebase** của SmartFarm: nhận thông báo do server của Phong gửi sang qua API (`POST /internal/notify`) và đẩy qua FCM tới màn hình kiosk cạnh cây (Firebase Web Push), hiển thị đúng nguyên nội dung, kể cả khi trang đóng. Mô hình "không dịch, chỉ đẩy push, targeting theo deviceId" giúp service đơn giản, dễ bảo trì và dễ mở rộng thêm client.

Công nghệ sử dụng: Node.js, Express, MongoDB, Firebase FCM.
